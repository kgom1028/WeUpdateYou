...
import com.kgom.refresh.internal.FlipLoadingLayout;
import com.kgom.refresh.internal.LoadingLayout;
import com.kgom.refresh.internal.RotateLoadingLayout;
import com.kgom.refresh.internal.Utils;
import com.kgom.refresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    static final float MAX_SCROLL_FRICTION = 3.0f;

    public static final int SMOOTH_SCROLL_DURATION_MS = 200;
    public static final int SMOOTH_SCROLL_LONG_DURATION_MS = 325;
    static final int DEMO_SCROLL_INTERVAL = 225;
    static final int DELTA_MOVE = 50;
    static final int ITEM_REMAIN_COUNT =2;
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (isReadyForPull()) {
                    ...
                    if (absDiff > mTouchSlop && (!mFilterTouchEvents || absDiff > Math.abs(oppositeDiff))) {
                        if (mMode.showHeaderLoadingLayout() && (getScrollY() <-1 || getScrollY() >=0 && diff >= 1f) && isReadyForPullStart()) {
                            mLastMotionY = y;
                            mLastMotionX = x;
    ...
    public final boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE: {
                if (mIsBeingDragged) {
                    float diff =  mLastMotionY - event.getY() ;

                    mDiffY += diff;
                    mLastMotionY = event.getY();
                    mLastMotionX = event.getX();

                    updateHeader(diff / 1.8f);
                    //pullEvent();
                    //pullEvent();
    ...
    protected  abstract int getMaxRefreshHeight(int remainItemCount);
    /**
     * Implemented by derived class to return whether the View is in a state
    ...
    private void pullEvent() {
        ...
        Log.d("refresh", "pullEvent: "+ newScrollValue);
        switch (getPullToRefreshScrollDirection()) {
            case VERTICAL:
                setHeaderScroll(getScrollY() + newScrollValue);
                break;
            case HORIZONTAL:
                setHeaderScroll(getScrollX() + newScrollValue);
                break;
        }
    ...
    private void updateHeader(float diffY)
    {
        final int itemDimension;
        itemDimension = getHeaderSize();

        float currentScrollValue = getScrollY();
        float newScrollValue = currentScrollValue+diffY;

        final int maximumPullScroll = getMaximumPullScroll();
        newScrollValue = Math.min(maximumPullScroll, Math.max(-maximumPullScroll, newScrollValue));
        if(newScrollValue > 0.1)
            newScrollValue = 0;
        if (mLayoutVisibilityChangesEnabled) {
            if (newScrollValue < 0) {
                mHeaderLayout.setVisibility(View.VISIBLE);
        ...
        if (USE_HW_LAYERS) {
             ...
             * often, which would negate any HW layer performance boost.
             */
            ViewCompat.setLayerType(mRefreshableViewWrapper, newScrollValue != 0 ? View.LAYER_TYPE_HARDWARE
                    : View.LAYER_TYPE_NONE);
        }

        int scroll= Math.round(newScrollValue);
        scrollTo(0,  scroll);
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            ...
            if (mState != State.RELEASE_TO_REFRESH &&  Math.abs(newScrollValue) > 0) {
                setState(State.RELEASE_TO_REFRESH);
            }/*else if (mState == State.PULL_TO_REFRESH && itemDimension < Math.abs(newScrollValue)) {
               // setState(State.RELEASE_TO_REFRESH);
            }*/
    ...
    private int getMaximumPullScroll() {
        switch (getPullToRefreshScrollDirection()) {
            ...
            case VERTICAL:
            default: {
                if (getMaxRefreshHeight(ITEM_REMAIN_COUNT) == 0)
                    return Math.round(getHeight() / FRICTION);
                else
                    return getMaxRefreshHeight(ITEM_REMAIN_COUNT);
            }
        }
    }
